...
void synthv1_dpf::run(const float **inputs, float **outputs, uint32_t nframes, const MidiEvent* midiEvents, uint32_t midiEventCount)
{
	...
	synthv1::process((float**)inputs, outputs, nframes);
}
...
SynthV1Plugin::SynthV1Plugin()
	: Plugin(synthv1::NUM_PARAMS, 0, 0) // parameters, programs, states
{
...
void SynthV1Plugin::run(const float** inputs, float** outputs, uint32_t frames, const MidiEvent* midiEvents, uint32_t midiEventCount) 
{
